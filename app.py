...
# ===============================
IMG_SIZE = 224
THRESHOLD = 0.4   # เน้น recall (cancer-safe)
...
def load_model():
    ...
    model.load_state_dict(
        torch.load("skin_cancer_attention.pth", map_location=DEVICE)
    )
    model.eval()
    return model
...
if uploaded_file is not None:
    ...
    if prob >= THRESHOLD:
        st.error("⚠️ **Malignant (มีความเสี่ยงเป็นมะเร็งผิวหนัง)**")
    else:
    ...
    st.info(
        ...
        "หากมีความกังวล ควรปรึกษาแพทย์ผู้เชี่ยวชาญ"
    )
